    ...
    >
      ...
      <div style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8 }}>
          <div style={{ fontWeight: 700, fontSize: 15, color: colors.ink }}>{item.name}</div>
          <div style={{ fontFamily: "var(--font-bricolage)", fontWeight: 800, fontSize: 16, color: colors.clay, whiteSpace: "nowrap" }}>{item.price}</div>
        </div>
        <div style={{ fontSize: 12.5, lineHeight: 1.5, color: colors.textMuted, fontStyle: "italic" }}>&ldquo;{item.note}&rdquo;</div>
